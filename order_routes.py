...
def criar_order(
    ...
    # 0) GARANTIR QUE O CLIENTE É DO OWNER ATUAL
    cliente = db.execute(
        select(Cliente)
    ...
    link_owner_id = cliente.link_owner_id  # pode ser None

    # 1) REGRA ANTIGA — impedir duplicidade (owner + cliente + codigo_pacote)
    existing = db.execute(
        select(Order)
        .where(Order.owner_id == owner_id)
        .where(Order.cliente_id == payload.cliente_id)
    ...
    # 2) NOVA REGRA — validar relação link_owner_id x orders existentes
    base_order_id: Optional[int] = None

    # Só faz a checagem se o cliente tiver link_owner_id definido
    if link_owner_id is not None:
        # todas as orders que já usam esse código, em qualquer owner
        orders_mesmo_codigo = db.execute(
            select(Order).where(Order.codigo_pacote == payload.codigo_pacote)
        ).scalars().all()

        if orders_mesmo_codigo:
            # verifica se existe algum owner diferente do link_owner_id
            owners_diferentes = {
                o.owner_id for o in orders_mesmo_codigo
                if o.owner_id != link_owner_id
            }

            if owners_diferentes:
                # já existe esse código em uma base fora da "rede" desse cliente
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Esse pacote já foi cadastrado para outra base fora da sua lista de clientes.",
                )

            # procura a order da BASE (owner_id == link_owner_id) para usar como base_order_id
            # se houver mais de uma, pegamos a mais recente
            base_order = sorted(
                (o for o in orders_mesmo_codigo if o.owner_id == link_owner_id),
                key=lambda o: (o.criado_em or datetime.min.replace(tzinfo=br_tz), o.order_id),
                reverse=True,
            )
            if base_order:
                base_order_id = base_order[0].order_id

    agora = datetime.now(br_tz)

    # 3) CRIAR ORDER — SEMPRE com user_id = current_user.user_id
    novo_order = Order(
        owner_id=owner_id,
        cliente_id=payload.cliente_id,
        codigo_pacote=payload.codigo_pacote,
        servico=payload.servico,
        status=3,  # conforme sua regra
        orders_rua=payload.orders_rua,
        orders_cep=payload.orders_cep,
        criado_em=agora,
        atualizado_em=agora,
        user_id=current_user.user_id,  # usuário logado
        base_order_id=base_order_id,   # <<--- NOVO: ordem da BASE, se existir
    )
    ...
    # 4) CRIAR PRIMEIRO EVENTO (status 3)
    primeiro_evento = OrderEvent(
        order_id=novo_order.order_id,
        tipo=3,
        actor_user_id=current_user.user_id,
...
def atualizar_order(
    ...
    if order.status == 1:
        ...
        if payload.cliente_id is not None and payload.user_id is None:
            ...
            if (cliente.tipo_cliente or "").upper() != "SUB_BASE":
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Cliente informado não é do tipo SUB_BASE.",
                )
